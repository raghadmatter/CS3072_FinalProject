...
```{r}
pred_crime <- filter(pred_crime, !is.na(violent_crime), !is.na(income))
```
...
```
```{r}
model_mars = train(total_crime ~ ., data=trainData, method='earth')
...
model_rf = train(total_crime ~ ., data=trainData, method='rf', tuneLength=5)
model_rf
...
model_svmRadial = train(total_crime ~ ., data=trainData, method='svmRadial', tuneLength=15, trControl = fitControl)
model_svmRadial
```
...
model_adaboost = train(total_crime ~ ., data=trainData, method='adaboost', tuneLength=2, trControl = fitControl)
model_adaboost
